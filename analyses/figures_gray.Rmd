...
```{r P-I-curves, echo = FALSE, message = FALSE, warnings = FALSE, cache = FALSE, fig.height=4, fig.width=5, fig.cap="(Gray scale) Photosynthesis-irradiance (P-I) curve for ecosystem metabolism. The dashed lines illustrate the contribution of each parameter of the P-I curve. The dotted line shows NEP of 0, where GPP equals ER (since NEP = GPP - ER)."}
pi_curve <- model_fit %>%
  ...
  mutate(middle = ifelse(name=="ER",-middle,middle)/1000,
         ...
         {ggplot(., aes(yday, middle))+
             ...
             geom_text(data = tibble(year = 2015, yday = 190, name = c("GPP","NEP","ER"), 
                                         middle = c(5,2,-3)), 
                       aes(label = name), hjust = 0, size = 3.5)+
...
```{r gpp-vs-er, echo = FALSE, message = FALSE, warnings = FALSE, cache = FALSE, fig.height=4, fig.width=4, fig.cap="(Gray scale) Estimated daily ER plotted against GPP, with the 1-to-1 line. The estimates of ER and GPP include the influence of temperature, which contributes to their correlation."}
model_fit %>%
...
```{r alpha, echo = FALSE, message = FALSE, warning = FALSE, cache = FALSE, fig.height=4, fig.width=5, fig.cap="(Gray scale) Initial slope of the P-I curve as inferred from the model, plotted through time. The lines are the posterior medians, and the shaded regions are the 68% uncertainty intervals matching the coverage of standard errors."}
# preapre data
alpha_d <- model_fit %>%
...
(ref:beta0-rho-cap) (Gray scale) Maximum GPP and baseline ER (at the reference temperature of 12$^\circ$C) as inferred from the model, plotted through time. The lines are the posterior medians, and the shaded regions are the 68% uncertainty intervals matching the coverage of standard errors.
...
(ref:beta0-vs-rho-cap) (Gray scale) Baseline ER plotted against maximum GPP, both defined at the reference temperature of 12$^\circ$C. Each line represents a separate year, with squares indicating the beginning and triangles the end of each series.
...
```{r beta0-phyc, echo = FALSE, message = FALSE, warnings = FALSE, cache = FALSE, fig.height=4, fig.width=5, fig.cap="(Gray scale) Maximum GPP inferred from the model and phycocyanin concentration (a cyanobacterial pigment), plotted through time. Both variables are shown as z-scores (centered on mean and divided by standard deviation across all days)."}
# prepare data
