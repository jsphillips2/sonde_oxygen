...
parameters{
  ...
  real log_beta0; // mean value for log_beta0
  real log_alpha; // mean value for log_beta0
  real log_rho; // mean value for log_rho
...
transformed parameters {
  ...
  // daily parameters
    // random walk
    for (d in 1:D){
      beta0[d] = exp(log_beta0 + sig_beta0*z_beta0[d]);
      alpha[d] = exp(log_alpha + sig_alpha*z_alpha[d]);
      rho[d] = exp(log_rho + sig_rho*z_rho[d]); 
    }
  // predicted oxygen 
  for (n in 1:N) {
