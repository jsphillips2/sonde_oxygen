...
parameters{
  ...
  real<lower=0> sig_opt; // sd of log_opt random walk 
  ...
  vector[Y+1] log_opt_init; // initial value for log_beta0
  ...
  vector[D] z_opt; // z value for non-centered parameterization
...
transformed parameters {
  ...
  vector[D] log_opt; // optimum light level (log scale)
  vector[D] log_rho; // er at temp_ref (log scale)
  vector<lower=0>[D] beta0; // max gpp at temp_ref [g m^-2 h^-1]
  vector<lower=0>[D] alpha; // 
  vector<lower=0>[D] opt; // optimum light level [um photons m^-2 s^-1]
  ...
  for (y in 1:Y) {
    ...
    log_opt[dy_st[y]] = log_opt_init[y];
    ...
    for (d in (dy_st[y]+1):(dy_st[y]+K[y]-1)){
      ...
      log_opt[d] = log_opt[d-1] + sig_opt*z_opt[d-1];
  ...
  for (n in 1:N) {
    ...
    gpp[n] = beta[n]*light[n]/
      ((beta[n]/(alpha[D_M[n]]*opt[D_M[n]]^2))*light[n]^2 + 
        (1 - 2*(beta[n]/(alpha[D_M[n]]*opt[D_M[n]])))*light[n] + 
          beta[n]/alpha[D_M[n]]);
