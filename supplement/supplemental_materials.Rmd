...
For fitting the model, I z-scored (centered on the mean and divided by the standard deviation) the observed DO concentrations and scaled the model accordingly. I then nondimensionalized the scaled model to remove the scale parameters (mixing depth and mean/standard deviation for the observed DO). This put the observed values and model parameters on unit scale, which improved the computational efficiency and allowed for the use for weakly-informative priors on unit scale. I then back-transformed the scaled parameters to recover their values on the original scale.
...
I partitioned variation in NEP, GPP, and ER resulting from variation in physical and biological parameters by applying the delta method to main text equations 2, 3, and 7, respectively. This method gives the first-order approximation of the variance in a function $F$ that depends on a vector $\Theta_{d,h}$ of time-varying parameters $\theta^1_{d,h}$, $\theta^2_{d,h}$, ... $\theta^k_{d,h}$ as

\begin{equation}
\text{Var}[F(\Theta_{d,h})] \approx \sum_{i}~\left( \frac{\partial \hat F}{\partial \hat {\theta^i}} \right)^2 \sigma^2_{\theta^i}~+~\sum_{i|i\neq j}~\sum_{j|j\neq i}~\frac{\partial \hat F}{\partial \hat {\theta^i}}~\frac{\partial \hat F}{\partial \hat {\theta^j}}~\sigma_{\theta^i \theta^j}
(\#eq:a)
\end{equation}

where $\hat{\theta^i}$ is the mean value of $\theta^i_{d,h}$ across all $d$ and h, $\hat F$ is the function evaluated at the vector of mean values for each parameter vector $\hat\Theta$, $\frac{\partial \hat F}{\partial \hat {\theta^i}}$ is the partial derivative of $\hat F$ with respect to  $\hat{\theta^i}$,  $\sigma^2_{\theta^i}$ is the variance in $\theta^i_{d,h}$, and $\sigma_{\theta^i \theta^j}$ is the covariance between $\theta^i_{d,h}$ and $\theta^j_{d,h}$. The relative contribution of a given parameter $\theta^{i=I}_{d,h}$ is calculated as the sum of all terms in equation \@ref(eq:a) where $i=I$, divided by the total variance $\text{Var}[F(\Theta_{d,h})]$. Note that the contribution of a parameter will be reduced by its negative covariances with other parameters and can even be negative if the contribution of its negative covariance terms exceeds the contribution of its (necessarily positive) variance term. The advantage of an explicit partitioning based on the model equations, as opposed to a more conventional variance partitioning (e.g., ANOVA), is that the explicit partitioning accounts for nonlinearities and interactions between explanatory variables through both derivatives and covariances. 

Generically, the contribution of a variable $\theta^i$ to the variance in $F$ is determined by (a) the sensitivity of $F$ to variation in $\theta^i_{d,h}$ (i.e. the partial derivatives in equation \@ref(eq:a)) and (b) the variance of $\theta^i_{d,h}$. This can be illustrated by considering the contribution of a single parameter $\theta^{i=I}_{d,h}$ in equation \@ref(eq:a)  relative to the total variance:

\begin{equation}
\left [\left( \frac{\partial \hat F}{\partial \hat {\theta^I}} \right)^2 \sigma^2_{\theta^I}~+~ \sum_{j|j\neq I} ~\frac{\partial \hat F}{\partial \hat {\theta^I}}~\frac{\partial \hat F}{\partial \hat {\theta^j}}~r_{\theta^I \theta^j}~\sigma_{\theta^I}~\sigma_{\theta^j} \right] /~ \text{Var}[F(\Theta_{d,h})]
(\#eq:b)
\end{equation}

where the covariance between a given pair of variables $\theta^I$ and $\theta^j$ is expressed in terms of their correlation ($r_{\theta^I \theta^j}$) and respective standard deviations ($\sigma_{\theta^I}$ and $\sigma_{\theta^j}$). Factoring $\sigma^2_{\theta^I}$ and expressing it in terms of the coefficient of variation ($\text{CV}_{\theta^I}$) yields 

\begin{equation}
\text{CV}^2_{\theta^I} \left (\hat{\theta^I} \right)^2 \left [\left( \frac{\partial \hat F}{\partial \hat {\theta^I}} \right)^2~+~ \sum_{j|j\neq I}~\frac{\partial \hat F}{\partial \hat {\theta^I}}~\frac{\partial \hat F}{\partial \hat {\theta^j}}~r_{\theta^I \theta^j}~\frac{\sigma_{\theta^j}}{\sigma_{\theta^I}} \right] /~ \text{Var}[F(\Theta_{d,h})]
(\#eq:c)
\end{equation}

which can be simplified by expressing it as the product of $\text{CV}^2_{\theta^I}$ and the “scaled sensitivity” $\psi_{\theta^I}$:

\begin{equation}
\text{CV}^2_{\theta^I}~\psi_{\theta^I}
(\#eq:d)
\end{equation}

Both $\text{CV}^2_{\theta^I}$ and $\psi_{\theta^I}$ are dimensionless and can be compared for different variables. $\text{CV}^2_{\theta^I}$ quantifies the overall variation in $\theta^I_{d,h}$, while $\psi_{\theta^I}$ quantifies how sensitive $F$ is to variation in $\theta^I_{d,h}$ given its correlations with other variables.
