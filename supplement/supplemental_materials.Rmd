...
flux_d <- lapply(list("main","alt_k"), 
                 function(x){read_csv(paste0("model/output/",x,"/summary_clean.csv")) %>%
                     ...
                     arrange(day, index) %>%
                     select(-day, -index) %>%
                     bind_cols(read_csv(paste0("model/input/",x,"/sonde_prep.csv")) %>% 
                                 na.omit() %>%
                                 arrange(year, yday, hour)) %>%
                     select(year, yday, hour, do, nep, air) %>%
                     group_by(year, yday) %>%
                     summarize(nep = 24*mean(nep, na.rm=T)/330,
                               air = 24*mean(air, na.rm=T)/330) %>%
                     gather(var, value, nep, air) %>%
                     mutate(analysis = x)}) %>% 
  bind_rows() %>%
...
# prepare data
flux_d %>%
  filter(!(var %in% c("flux","error"))) %>%
  ggplot(aes(yday, value, color = analysis, linetype = var))+
  facet_wrap(~year)+
  geom_line(size = 0.5)+
  geom_hline(yintercept = 0, size = 0.3, alpha = 0.5)+
  scale_color_manual("",values=c("firebrick","dodgerblue"))+
  scale_linetype_discrete("")+
  scale_y_continuous(expression(Daily~DO~Flux~"("*mg~O[2]~m^{-2}~d^{-1}*")"))+
